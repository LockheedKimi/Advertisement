...
@interface BannerPictureView ()<UIScrollViewDelegate>
...
@implementation BannerPictureView
@synthesize pageControll = _pageControll;

+ (instancetype)shareInstaceWithFrame:(CGRect)frame images:(NSArray<UIImage *> *)images isAutoScroll:(BOOL)isAuto {
    static BannerPictureView *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[BannerPictureView alloc] initWithFrame:frame images:images isAutoScroll:isAuto];
    });
    return instance;
}
...
{
    BannerPictureView *picView = [[BannerPictureView alloc] init];
    NSInteger num = urlStrings.count;
    if(urlStrings.count == 0) {
        num = 1;
    }
    NSMutableArray *images = @[].mutableCopy;
    for (int j = 0; j < num; j++) {
        UIImage *image = [UIImage imageNamed:@"banner_bg"];
        image=[image stretchableImageWithLeftCapWidth:0 topCapHeight:j];
    ...
    dispatch_async(queue, ^{
        [urlStrings enumerateObjectsUsingBlock:^(NSString * _Nonnull urlString, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                ...
                if (i >= urlStrings.count) {
                    i = 0;
                    picView.pageControll.numberOfPages = images.count;
        ...
        //            block([HGScrollPictureView shareInstaceWithFrame:frame images:images isAutoScroll:isAuto]);
        //        });
    });
    dispatch_async(dispatch_get_main_queue(), ^{
        block([BannerPictureView shareInstaceWithFrame:frame images:images isAutoScroll:isAuto]);
    });
...
- (void)addImagesView
{
    ...
    //        self.currentImageView.contentMode = UIViewContentModeCenter;
    //   [self.scrollView addSubview:self.currentImageView];
    ...
    dispatch_async(queue, ^{
        [urlStrings enumerateObjectsUsingBlock:^(NSString * _Nonnull urlString, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                static int i = 0;
                if(image!=nil) {
                    image=[image stretchableImageWithLeftCapWidth:0 topCapHeight:0.001*i];
                    [images replaceObjectAtIndex:idx withObject:image];
                }
                i++;
                if (i >= images.count) {
            ...
            [self sd_setImageLoadOperation:operation forKey:[NSString stringWithFormat:@"ImageLoad%ld",idx]];
        }];
        dispatch_semaphore_wait(sem, DISPATCH_TIME_FOREVER);
        if (!images) {
            return;
